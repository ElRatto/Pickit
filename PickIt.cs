...
namespace PickIt
{
    public class PickIt : BaseSettingsPlugin<PickItSettings>
    {
        ...
        private IEnumerator MainWorkCoroutine()
        {
            while (true)
            {
                yield return Metamorhosispicker();
        ...
        private IEnumerator Metamorhosispicker()
        {
            ...
            List<Tuple<int, Vector3>> currentLabels = GameController.Game.IngameState.IngameUi.ItemsOnGroundLabels.ToList()
                                                ...
                                                .OrderBy(x => x.Item1)
                                                .ToList();
            Tuple<int, Vector3> pickUpThisItem = (from x in currentLabels where x.Item1 < Settings.PickupRange select x).FirstOrDefault();
            if (pickUpThisItem != null)
            {
                //LogMessage("try to pick" + pickUpThisItem.Item2.ToString(), 5);
                //TryToPick2(pickUpThisItem.Item2);
                Camera camera2 = GameController.Game.IngameState.Camera;
                Vector2 entityPosToScreen2 = camera2.WorldToScreen(pickUpThisItem.Item2.Translate(0, 0, 0));



                //FinalMouseFunction(entityPosToScreen2, true, false);
                Mouse.MoveCursorToPosition(entityPosToScreen2);
                yield return _wait3Ms;
                Mouse.MoveCursorToPosition(entityPosToScreen2);
                yield return _wait3Ms;
                yield return Mouse.LeftClick();
